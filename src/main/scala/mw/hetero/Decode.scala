...
object Decode {
	...
	implicit def tuple2[T1, T2](implicit decode1: Decode[T1], decode2: Decode[T2]): Decode[(T1, T2)] =
		new Decode[(T1, T2)] {
			def minSize = decode1.minSize + decode2.minSize
	...
	Decode[(T1, T2, T3)] = new Decode[(T1, T2, T3)] {
		def minSize = decode1.minSize + decode2.minSize + decode3.minSize
		def apply(data: List[String]) = {
			val (t1, rest1) = decode1(data)
			val (t2, rest2) = decode2(rest1)
	...
	implicit def hList[H, T <: HList](implicit headDecode: Decode[H], tailDecode: Decode[T]): Decode[H :: T] =
		new Decode[H :: T] {
			def minSize = headDecode.minSize + tailDecode.minSize
			def apply(data: List[String]) = {
	...
	implicit val hNil: Decode[HNil] = new Decode[HNil] {
		val minSize = 0
		def apply(data: List[String]) = (HNil, data)
	}
	implicit val range: Decode[Range] = new Decode[Range] {
		val minSize = 2
		def apply(data: List[String]) = {
			val (from, rest1) = int(data)
			val (to, rest2) = int(rest1)
			(Range(from, to), rest2)
		}
	}
}
